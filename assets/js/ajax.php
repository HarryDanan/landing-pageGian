<script type="text/javascript">
	...
	function tampilKoperasiAuth() {
		$.get('<?php echo base_url('Auth/tampil'); ?>', function(data) {
			MyTable.fnDestroy();
			$('#data-koperasiAuth').html(data);
			refresh();
		});
	}
	...
	$(document).on("click", ".view-dataKoperasi", function() {
		var id = $(this).attr("data-id");

		$('#tempat-modal').html(data);
		$('#view-koperasi').modal('show');
	})
	
	$(document).on("click", ".update-dataKoperasi", function() {
		var id = $(this).attr("data-id");

		$.ajax({
				method: "POST",
				url: "<?php echo base_url('Koperasi/update'); ?>",
				data: "id=" + id
			})
			.done(function(data) {
				$('#tempat-modal').html(data);
				$('#update-koperasi').modal('show');
			})
	})
